...
**Idempotency Base** (`EdictIdempotencyBase`):
The abstract base that Event Handlers, Sagas, and Projection Builders inherit (the shared inheritance root — brand-rule clause b). Its sole job is idempotency: a bounded per-grain ring of recently seen `EventId`s that suppresses at-least-once redeliveries. It does **not** decide which stream to subscribe to — the consuming type declares that itself.
