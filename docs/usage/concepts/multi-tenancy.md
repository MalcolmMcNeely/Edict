...
Establishing a crossing is **origin-only**. A relayed send — a saga `Dispatch`, an outbox `SendCommand`, a schedule fire — inherits the tenant of the message it is reacting to; it cannot mint one. So there is no saga-side establishing overload: a chain that needs to cross into a tenant must do so at an origin send that names the tenant, never mid-relay. A relayed send that reaches a tenant-scoped aggregate carrying no tenant is not a silent crossing — it fails closed (see below).
...
- **The same refusal on a relayed send.** A saga `Dispatch`, an outbox `SendCommand`, or a schedule fire is exempt from the origin stamper — it inherits the tenant of the message it relays rather than re-resolving one — so an origin throw cannot catch a relay that lost its tenant. The send paths therefore re-check at the composition site: a relayed command reaching a tenant-scoped aggregate while carrying a null tenant throws `EdictMissingTenantException` *before* a key is composed, so it **dead-letters** rather than landing a bare key in the shared default partition. The throw is not seen by a consumer (a relay has no caller to surface it to); it flows to the outbox catch and the row appears in the dead-letter log, tagged with the missing-tenant cause. An intra-tenant relay (the inherited tenant is non-null) and a relay into a public aggregate (no wall to breach) both pass untouched.
