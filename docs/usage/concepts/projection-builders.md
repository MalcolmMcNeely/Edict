...
- **`EdictProjectionBuilder`** (`Edict.Core.Projections`) — abstract marker base for the projection-builder role. Inherits the dedup ring and the implicit stream subscription from `EdictIdempotencyBase`. A consumer declares a `partial class` (the generator emits the dispatch switch and the stream subscription attribute) and writes one `Task Handle(TEvent edictEvent)` per subscribed event type.
...
- **EDICT009** — every `Handle` method must return `Task` (not `Task<T>`) and take a single parameter that derives from `EdictEvent`.
