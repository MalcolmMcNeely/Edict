...
- **A wall-clock staleness heuristic** that picks child vs link by how late a drain runs — rejected as complexity for no benefit. Child-vs-link is not a question of elapsed time; it is the plain per-turn invariant, decided at span-start by whether the drain still holds the live event reference. An inline drain publishes in the same turn that raised the event — it is not a cross-turn hop, so it stays parent-child. A recovery drain (reminder / activation) has only the rehydrated `traceparent`: it is a genuinely later, possibly cross-silo turn, so it links. No staleness measurement is involved, and the live-ref signal is exact, so the recovery-drain row in the span map above (root + link) and the inline publish nested under its command turn (the parent-child `edict.event.publish` in the API-command row) are two faces of one rule, not a contradiction.
