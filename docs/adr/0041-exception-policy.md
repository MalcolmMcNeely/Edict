...
The remedy splits by lifecycle. A **wiring fault** — knowable before the silo runs (missing client, missing options, missing provider marker, duplicate registrar) — throws `EdictWiringException` from the extension call site or from `EdictWiringValidator` at host start. Never from a `Handle` call. A **runtime fault** throws a per-cause-narrative `Edict*` exception (`EdictUnregisteredTypeException`, `EdictClaimCheckFetchException`, `EdictSagaCoordinationException`, `EdictInternalInvariantException`, etc.), is caught by `OutboxHost.ExecuteGroupCapturingAsync`, classified by `DeadLetterFailureClassifier`, and dead-lettered per ADR 0018.
