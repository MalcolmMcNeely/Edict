...
- Substrate **driver** faults (raw Npgsql / Azure / Kafka exceptions that are not `Edict*`-typed) are classified outside this policy: each provider registers an `IDeadLetterFaultClassifier` that `DeadLetterFailureClassifier` consults on fallthrough, rather than `Edict.Core` name-matching a provider it cannot reference. See ADR-0052.
