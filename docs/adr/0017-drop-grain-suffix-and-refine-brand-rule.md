...
3. **Refined brand rule.** A type is `Edict`-prefixed if and only if **(a)** a consumer types it (derives from it, applies it as an attribute, or receives/returns it), **or (b)** it is an inheritance root shared by the consumer-facing grain bases. Clause (b) admits **multiple roots** when the hierarchy genuinely has nested shared roots: today the outer shared root is **`EdictDurableConsumerBase<TPayload>`** (the single home for the `IOutboxHost` adapter, the lazy drain Reminder, the dead-letter admin, drain-on-activation, and the intake-block guard — the plumbing that used to be duplicated between the two consumer-facing roots), and **`EdictIdempotencyBase<TPayload>`** retains its clause-(b) status as the root shared by the idempotent-consumer family (event handlers, projection builders, sagas). Both keep the prefix under (b) even though no consumer derives from them directly — they sit in every consumer grain's base chain, so the prefix still signals "framework contract" and reads coherently in the hierarchy.
