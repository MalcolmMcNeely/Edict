...
- Two lifecycle counters land on the `"Edict"` Meter: `edict.saga.timeout.fired` (tagged `compensated`, `deadlettered`, or `compensation_failed`) and `edict.saga.completed`. The ratio `fired / (fired + completed)` separates a handful of timeouts among millions from a rising failure trend. The existing `edict.saga.progress.age` gauge stays as the leading indicator of a saga approaching its cap.

- `EdictSagaTimeoutException` and `EdictSagaTerminalException` join the dead-letter failure-reason allowlist as `SagaTimeout` and `SagaTerminal`, distinct from `ConsumerBug`, so the two saga-lifecycle failures read apart on the dead-letter projection. A throwing override is a third cause: `EdictSagaCompensationException` maps to `ConsumerBug`, because a hook that threw is a fault in the consumer's compensation code, not a by-design stall — it reads apart from the `SagaTimeout` (no override / requested dead-letter) row.

- **A throwing `OnSagaTimeoutAsync()` override is contained, not retried.** When the cap fires and the consumer's override throws, the cap handler catches it, reloads the durable snapshot to roll back any half-applied `Progress` mutation and discard any buffered compensating Command, terminalises the saga to `TimedOut`, dead-letters the failure as `EdictSagaCompensationException` (classified `ConsumerBug`, carrying the originally-thrown exception as `InnerException` and its type and message in `Reason`), emits `edict.saga.timeout.fired` with outcome `compensation_failed`, and unregisters the cap reminder so it fires exactly once. Without containment the throw escapes before the terminal write and the unregister, leaving the saga `Live` with its cap armed and re-throwing on every reminder tick: a silent poison-loop that strands the workflow. Auto-retrying the throw is rejected for the same reason a fired cap is not auto-retried (above): a deterministic consumer throw would re-throw forever. The narrow exception is a storage fault *during* the rollback reload, which is left to escape and self-resolve under reminder retry, because a storage outage is transient where a consumer throw is not.
