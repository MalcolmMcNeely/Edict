...
|---|---|---|---|
| 4 | **Tenant ADR stub + CONTEXT glossary entry** | `docs/adr/`, `CONTEXT.md` | Commit the *envelope-carry-not-ambient* decision now (no runtime code). **Resolve one fork before stubbing: the `EdictTenantId` type.** The earlier "`Guid`-backed" note is now contested — `theorycraft-multi-tenant-identity-and-surface.md` (Open Question 4) shows the auth story yields a `Guid` only for B2B Entra `tid`; Entra External ID custom claims and non-Microsoft IdPs emit arbitrary string slugs, so a `Guid` constraint quietly excludes viable auth stories and breaks the "generic regardless of auth" property. Lean opaque, charset-validated `string` (charset validation is load-bearing: the value becomes part of partition keys / blob paths) with a `Guid` fast-path internally. This type is the *one* piece that becomes a later break if frozen wrong; decide it before stubbing. This contradicts the sibling `theorycraft-tenant-scoped-substrate.md` (Open Question 2, which recommends `Guid`); the ADR reconciles the two. Add one line to the dead-letter ADR (0018) pre-acknowledging the fleet-wide poison read may later become a privileged cross-tenant path, so the eventual change reads as planned evolution, not a 1.x behavior break. |
...
| **Tenant-scoped substrate** | **Deferral verified safe (2026-06-05 code audit).** The load-bearing invariant is *placement* identity, not payload-byte identity; the two were confirmed separately. (1) **Placement** — every grain role is `IGrainWithGuidKey` + a bare `[EdictRouteKey]` Guid (`EdictSender.cs:39`, `IEdictCommandHandler.cs:12`, `IEdictEventConsumer.cs:22`); no compound/string/hash key exists anywhere, so the tenancy-off path keeps today's exact grain key, and all 9 downstream placement surfaces (stream id, Postgres + Azure grain state, both claim-check stores, table-projection rows, dead-letter partition) derive from that Guid and stay byte-identical when off. (2) **Payload** — wire types are MessagePack map-mode (`keyAsPropertyName`, a new field writes a `:null` key, so NOT byte-identical) but are transient; persisted state is Orleans `[Id]`-tagged (`GrainEnvelope` highest `[Id(4)]`, appends at `[Id(5)]`) and version-skew-tolerant, so a tenancy-capable Edict reads an existing single-tenant deployment's state with no migration. Re-keying storage only strands *data at rest*, of which there is none pre-release. | See hedge #4. Carry side + auth story: `theorycraft-multi-tenant-identity-and-surface.md`; honour side: `theorycraft-tenant-scoped-substrate.md`. **ADR design note from the audit:** carry tenant on the framework envelopes (`GrainEnvelope` `[Id(5)]`, `EdictEventEnvelope`), NOT the `EdictCommand`/`EdictEvent` consumer bases, so consumer wire shapes stay byte-stable. The dead-letter `"deadletter"` literal partition (`EdictDeadLetterProjectionBuilder.cs:18`) is the one surface whose later scoping changes a *documented operator behavior*. |
