...
**Sibling docs:**
- [`theorycraft-multi-tenancy.md`](theorycraft-multi-tenancy.md) — shares both the **carry mechanism** and the **dead-letter-as-cross-cutting-store** problem. The multi-tenancy carry now explicitly reuses this audit feature's shipped principal carry (one resolver seam, one envelope-capture point, one per-turn relay), so the tenant id enters exactly the way the `principal` already does: an authenticated claim resolved at the edge, stamped onto the envelope, never trusted from a command body. The dead-letter row is the closest thing Edict has to an audit record (`PayloadJson` plus source metadata) and the sharpest GDPR liability (materialised personal data pooled fleet-wide); the multi-tenancy doc owns scoping it and flags the same ADR-0018 amendment this doc cares about.
