...
// Mid-batch crash variant of KafkaSiloKilledMidHandlerTests: two events for the
// same aggregate are published back-to-back, so both land at the only receiver
// for the stream's single partition with their offsets uncommitted. The first
// delivery blocks inside Handle; the silo is killed before the pulling agent's
// MessagesDeliveredAsync fires. Because the Edict.Kafka receiver commits offsets
// only inside MessagesDeliveredAsync, neither event's offset has advanced — on
// restart the new consumer reads from the pre-batch offset and re-delivers BOTH
// events. Each one runs through Handle exactly once, so the reference-store
// projection row settles at Count = 2.
//
// What's being pinned: the EdictKafkaReceiver's commit-after-handle ordering, the
// enable.auto.commit=false floor, and the ring + UpsertRow atomic write — together
// they make a mid-batch crash safe for the tail of the batch, not just the
// in-flight event at the head.
...
public sealed class KafkaSiloKilledMidBatchTests(KafkaSiloKillClusterFixture fixture)
{
    ...
    public async Task ProjectionHandle_ShouldWriteRowOncePerEvent_WhenHostingSiloKilledMidBatch()
    {
        ...
        var row = await WaitForRowAsync(aggregateId, expectedCount: 2);
    ...
    async Task<KafkaSiloKillTableRow?> WaitForRowAsync(Guid aggregateId, int expectedCount)
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            var row = await fixture.GetProjectionRowAsync<KafkaSiloKillTableRow>(
                KafkaSiloKillBatchProjectionBuilder.Table, aggregateId);
