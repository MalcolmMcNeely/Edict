...
/// the consumer's <c>HandleAsync(TEvent)</c> inline. The actual invocation runs
...
/// <c>partial</c> class with one <c>HandleAsync(TEvent)</c> overload per event type,
...
/// inside <c>HandleAsync</c>; every throw is treated uniformly as transient → exp
...
public abstract class EdictEventHandler : EdictIdempotencyBase
{
    ...
    /// concrete subclass has a matching <c>HandleAsync(TEvent)</c> overload for
