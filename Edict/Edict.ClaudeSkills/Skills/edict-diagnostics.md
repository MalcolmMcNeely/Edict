...
- **A dead-letter row carries `ExceptionType = EdictMissingTenantException`** — unlike the origin throw above, this one *is* a dead-letter, and it means a **saga crossed into a tenant aggregate without a tenant; establishing is origin-only**. A relayed send (a saga `Dispatch`, an outbox `SendCommand`, a schedule fire) inherits the tenant of the message it reacts to rather than re-resolving one, so it is exempt from the origin stamper — which is why the refusal cannot fire at an edge. The send paths instead re-check at the composition site: a relayed command reaching a tenant-scoped aggregate with a null tenant is refused before a key is composed, so it dead-letters rather than landing a bare key in the shared default partition. The fix is not in the relay: a chain that legitimately needs to cross into a tenant must establish the crossing at an **origin** send that names the tenant (`SendAsync(command, EdictTenantId.Of(...))`), because the public-origin event the saga was reacting to carried no tenant to inherit. An intra-tenant relay (the inherited tenant is non-null) and a relay into a public aggregate never produce this row.
