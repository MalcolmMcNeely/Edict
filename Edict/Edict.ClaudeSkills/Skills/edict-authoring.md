...
A feature authored under an audited silo (`silo.WithAudit()`) is attributed automatically: the **principal** is stamped at the originating `SendAsync` and the handler turn, its raised Events, and any Saga `Dispatch` it triggers all inherit it. You never thread an actor parameter through a handler or add a principal field to a Command — see the `edict-contracts` skill for why it is framework-managed, not authored.

The principal must name whoever actually authorized the action, and which origin you are writing decides how it is supplied. The choice is per-origin, not per-app: a single app routinely has both an edge resolver for its user traffic and a minted system principal for its background work.

- A **user-initiated send** at an authenticated edge (an HTTP request, a UI action) must resolve the real actor. Wire an edge resolver so the user's decision is recorded under the user, and leave the bare `SendAsync(command)` at the edge — the resolver attributes it. Stamping a constant or `system` principal on a user-initiated send is the severe failure to avoid: it records a real person's decision under a fabricated actor, and no later query can recover who actually acted.
- An **actor-less origin** you write yourself — a background worker, an import, a scheduled job, an admin script — has no ambient identity to resolve, so a constant `system` principal is correct and expected: supply one explicitly with `SendAsync(command, EdictPrincipal.Of("system"))`. This is honest attribution for work no user initiated, not a shortcut.
