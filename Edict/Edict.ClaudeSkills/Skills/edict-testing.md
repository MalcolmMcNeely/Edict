...
- **`EdictTestApp.Audit`** — the consumer read surface (`IEdictAuditRepository`) over the captured chain: `ByEntityAsync` / `ByConversationAsync` / `ByPrincipalAsync`, `VerifyEntityChainAsync`, and `GetPayloadAsync`. Available only after `WithAudit()`.
