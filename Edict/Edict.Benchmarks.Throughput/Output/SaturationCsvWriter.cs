...
public static class SaturationCsvWriter
{
    public static string Render(IReadOnlyList<SaturationResults> results)
    {
        ...
        sb.AppendLine("substrate,species,events_per_second,window_seconds,producer_concurrency,aggregate_count,succeeded,failed,failure_rate,failure_types");
        foreach (var r in results)
        {
            sb.AppendLine(CultureInfo.InvariantCulture,
                $"{r.Substrate},{r.Species},{r.EventsPerSecond:F2},{r.WindowSeconds:F2},{r.ProducerConcurrency},{r.AggregateCount},{r.Health.Succeeded},{r.Health.Failed},{r.Health.FailureRate:F4},{r.Health.RenderFailureTypes()}");
        }
        return sb.ToString();
    }
