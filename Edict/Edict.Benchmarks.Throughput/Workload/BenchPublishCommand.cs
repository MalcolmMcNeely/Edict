...
/// <para>
/// <c>PollKey</c> is a per-send token minted by the benchmark issuer.
...
/// </para>
/// </summary>
public sealed partial record BenchPublishCommand(Guid AggregateId, Guid PollKey, byte[] Filler) : EdictCommand
{
    [EdictRouteKey]
    public Guid AggregateId { get; init; } = AggregateId;

    public Guid PollKey { get; init; } = PollKey;
