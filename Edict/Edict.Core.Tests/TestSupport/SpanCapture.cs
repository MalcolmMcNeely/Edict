...
// Captures Edict spans for assertion. ActivityStopped fires when a span's
// using-scope unwinds; for a deferred handle, invocation, or schedule-fire span
// that is a separate, later signal than any handler or event-count probe a test
// might await. Waiting on a probe and then querying the list races the span
// emission, so tests must wait on the span they assert on via WaitForSpanAsync.
// Add and read are synchronized because ActivityStopped fires on arbitrary grain
// threads while the test enumerates.
sealed class SpanCapture : IDisposable
{
