...
// Records every published event off the outbox seam so a test can read the tenant
// the inline drain stamped onto the wire, without a real subscriber. Its own
// static (distinct from the audit executor's) keeps the tenant collection isolated
// from collections that run in parallel.
sealed class TenantCapturingExecutor(Serializer serializer) : IOutboxEffectExecutor
{
