...
/// partition via any provider's <see cref="Contracts.TableStorage.IEdictTableRepository{T}"/>.
/// The four facts pin the read contract: grain-keyed filter, missing-key empty,
/// list-all, and never-created-table empty.
/// </summary>
    ...
    where TFixture : PersistenceConformanceFixture
{
    ...
    public async Task ListAsync_ShouldReturnEntriesMatchingGrainKey()
    {
        ...
        var repository = NewRepository(tableName);

        var results = await repository.ListAsync("grain-A");
    ...
    public async Task ListAsync_ShouldReturnEmpty_WhenNoEntriesMatchGrainKey()
    {
        ...
        var results = await repository.ListAsync("grain-missing");
    ...
    public async Task ListAllAsync_ShouldReturnEmpty_WhenPartitionEmpty()
    {
        ...
        var repository = NewRepository(UniqueTable());
