...
/// <summary>
/// Receiver-side claim-check conformance for a saga: a pointer-bearing
/// <c>EdictEventEnvelope</c> reaches <c>ClaimCheckSaga</c> via the real
/// substrate stream and dispatches through the deferred <c>InvokeHandler</c>
/// path. The saga's staged <c>SendCommand</c> effect must survive that path —
/// the follow-up command lands at its handler and <c>Progress</c> is persisted,
/// exactly as for an in-band event.
/// </summary>
