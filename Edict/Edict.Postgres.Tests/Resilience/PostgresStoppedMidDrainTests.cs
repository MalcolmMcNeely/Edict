...
// The Postgres connection-drop fault mode, exercised against the outbox drain. A
// one-shot synthetic publish failure only stages a durable pending outbox entry
// and arms the drain reminder; the fault under test is the real Postgres outage on
// the recovery drain's state write-back. With Postgres stopped the recovery drain
// publishes the event to the dumb MemoryStreams reference but cannot persist the
// ack, so the entry stays pending and the activation is dropped. Once Postgres
// restarts, a reminder tick reconnects, the ack persists, and the outbox settles
// to empty with the reminder unregistered.
//
// Because the publish precedes the Postgres ack-write, a drain-window outage always
// lands post-publish and the recovery re-drain re-publishes. EventId is assigned
// once as the event enters the Outbox and carried on the persisted payload, so that
// re-publish carries the same id and the consumer dedup ring collapses it across
// the two deliveries the deliver-once reference stream made: this pins
// effectively-once across the ack-write-window fault on a real store, not just
// at-least-once.
...
public sealed class PostgresStoppedMidDrainTests(PostgresResilienceClusterFixture fixture)
{
    ...
    public async Task DrainRecovery_AppliesExactlyOnce_WhenPostgresStoppedMidDrainForcesRepublish()
    {
        ...
        // re-publishes the same event (a fresh publish over the still-pending entry).
        ...
        // The re-published event carries the same EventId as the original publish,
        // so the consumer's dedup ring collapses it: the handler applies exactly
        // once across the publish / re-publish, not twice.
