...
/// composed stream key. Pure data — no Orleans, no DI — so the
...
/// <param name="RouteKeyGetter">
/// Reads the event's composed stream key: its <c>[EdictRouteKey]</c> route key
/// stringified and folded through <see cref="EdictKeyComposer"/>.
/// </param>
public sealed record EdictEventStreamAccessor(
    string StreamName,
    Func<EdictEvent, string> RouteKeyGetter);
